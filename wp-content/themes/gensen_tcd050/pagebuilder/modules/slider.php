...
function page_builder_widget_slider_sctipts() {
	if (apply_filters('page_builder_slider_slick_script_load', true)) {
		...
		if ($pb_slick_script_path) {
			$pb_slick_script_path = str_replace(ABSPATH, home_url('/'), $pb_slick_script_path);
			$pb_slick_script_path = apply_filters('page_builder_slider_slick_script_url', $pb_slick_script_path);
			if ($pb_slick_script_path) {
				wp_enqueue_script('slick-script', $pb_slick_script_path, array('jquery'), PAGE_BUILDER_VERSION, true);
			}
		}
	}
}
...
function page_builder_widget_slider_styles() {
	if (apply_filters('page_builder_slider_slick_style_load', true)) {
		...
		if ($pb_slick_style_path) {
			$pb_slick_style_path = str_replace(ABSPATH, home_url('/'), $pb_slick_style_path);
			$pb_slick_style_path = apply_filters('page_builder_slider_slick_style_url', $pb_slick_style_path);
			if ($pb_slick_style_path) {
				wp_enqueue_style('slick-style', $pb_slick_style_path, false, PAGE_BUILDER_VERSION);
			}
		}
	}
